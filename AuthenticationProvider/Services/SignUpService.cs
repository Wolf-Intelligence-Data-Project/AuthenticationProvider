...
public class SignUpService : ISignUpService
{
    ...
    public async Task<SignUpResponse> RegisterCompanyAsync(SignUpRequest request)
    {
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError($"Error occurred during company registration. Error: {ex.Message}");

            // Rethrow a user-friendly message (in Swedish) without sensitive details
            throw new InvalidOperationException("Det uppstod ett problem vid registreringen av företaget. Försök igen senare.", ex);
        }
    }
}
